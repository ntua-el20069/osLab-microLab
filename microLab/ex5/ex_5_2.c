...
int main(void) {
 ...
 // maybe a small delay here or not ? 
 // _delay_ms(50);
 keyboard_input = PCA9555_0_read(REG_OUTPUT_1);
 
 // if "*" seen, then LED PD4 on
 if((keyboard_input & STAR_MASK) == STAR_MASK)      // I supposed that pressed means 1 (I do not know exactly)
    PCA9555_0_write(REG_OUTPUT_0, LED_PD4_ON);

 
 // if "0" seen, then LED PD5 on
 else if((keyboard_input & ZERO_MASK) == ZERO_MASK)
    PCA9555_0_write(REG_OUTPUT_0, LED_PD5_ON);

 
 // if "#" seen, then LED PD6 on
 else if((keyboard_input & HASHTAG_MASK) == HASHTAG_MASK)
    PCA9555_0_write(REG_OUTPUT_0, LED_PD6_ON);

 
 // if "D" seen, then LED PD7 on
 else if((keyboard_input & D_MASK) == D_MASK)
